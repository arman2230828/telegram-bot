...
def home_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("☁️ Upload File", callback_data="upload_file"),
...
def force_join_keyboard(channels: list, bot_username: str = None) -> InlineKeyboardMarkup:
    ...
    buttons.append([InlineKeyboardButton("✅ I've Joined — Check Now", callback_data="verify_join")])
...
def my_files_keyboard(files: list, offset: int, total: int, per_page: int = 10) -> InlineKeyboardMarkup:
    ...
    nav = []
    if offset > 0:
        nav.append(InlineKeyboardButton("← Prev", callback_data=f"my_files_{offset - per_page}"))
    if offset + per_page < total:
        nav.append(InlineKeyboardButton("Next →", callback_data=f"my_files_{offset + per_page}"))
    if nav:
        buttons.append(nav)
...
def search_results_keyboard(files: list, query: str, offset: int, total: int, per_page: int = 10) -> InlineKeyboardMarkup:
    ...
    nav = []
    if offset > 0:
        nav.append(InlineKeyboardButton("← Prev", callback_data=f"search_page_{query}_{offset - per_page}"))
    if offset + per_page < total:
        nav.append(InlineKeyboardButton("Next →", callback_data=f"search_page_{query}_{offset + per_page}"))
    if nav:
        buttons.append(nav)
    buttons.append([InlineKeyboardButton("« Back to Home", callback_data="home")])
    return InlineKeyboardMarkup(buttons)
...
def file_info_keyboard(unique_code: str, is_owner: bool = False) -> InlineKeyboardMarkup:
    buttons = [
        [InlineKeyboardButton("⬇️ Download File", callback_data=f"dl_{unique_code}")],
    ]
    if is_owner:
        buttons.append([InlineKeyboardButton("🗑 Delete File", callback_data=f"del_{unique_code}")])
...
def broadcast_schedule_keyboard(target: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⚡ Send Now", callback_data=f"bcast_schedule_now_{target}")],
...
def edit_texts_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🚫 Access Denied Message", callback_data="edittext_access_denied")],
        [InlineKeyboardButton("✅ Verify Success Message", callback_data="edittext_verify_success")],
        [InlineKeyboardButton("❌ Verify Failed Message", callback_data="edittext_verify_failed")],
